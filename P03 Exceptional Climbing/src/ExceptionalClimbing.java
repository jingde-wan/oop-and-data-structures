...
public class ExceptionalClimbing {
   ...
   * @throws IllegalArgumentException if the grade input is invalid or the provided array is full
   * @throws DataFormatException if provided array has null elements between index 0 (inclusive) and index
numSend, or passed negative value for numSend/numFail
   */
  public static int sendClimb(String[] send, int numSend, String grade) 
    ...
    if (numSend == send.length)
    	throw new IllegalArgumentException("cannot add new value to full length " + send.length + " array"); // it's full

    
    // check null elements of array or negative numSend
...
numSend, or passed negative value for numSend/numFail
  ...
  public static String getStats(String[] send, int numSend, String[] fail, int numFail, 
	...
	// check historyLength is 0 or negative
	if (historyLength <= 0) {
